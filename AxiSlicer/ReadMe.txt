================================================================================
    MFC 라이브러리 : AxiSlicer 프로젝트 개요
================================================================================

응용 프로그램 마법사에서 이 AxiSlicer 응용 프로그램을 
만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 
프로그램작성을 위한 기본 구조를 제공합니다.

이 파일에는 AxiSlicer 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
AxiSlicer.h
    ...
    헤더(Resource.h 포함)가 들어 있고 CAxiSlicerApp 응용 프로그램
    클래스를 선언합니다.

AxiSlicer.cpp
    응용 프로그램 클래스 CAxiSlicerApp이(가) 들어 있는 기본 응용 프로그램
    소스 파일입니다.
...
res\AxiSlicer.ico
    ...
    주 리소스 파일인 AxiSlicer.rc에 의해 포함됩니다.
...
AxiSlicerDoc.h, AxiSlicerDoc.cpp - 문서
    이 파일에는 CAxiSlicerDoc 클래스가 들어 있습니다. 이 파일을 편집하여
    특수한 문서 데이터를 추가하고 CAxiSlicerDoc::Serialize를 통해 파일
    저장 및 로드를 구현합니다.
    이 문서에는 다음 문자열이 포함됩니다.
        파일 확장명:      STL-AxialSpace
        파일 형식 ID:        AxiSlicer.Document
        주 프레임 캡션:  AxiSlicer
        문서 형식 이름:       AxiSlicer
        필터 이름:         AxiSlicer Files (*.STL-AxialSpace)
        파일의 새 약식 이름: AxiSlicer
        파일 형식의 긴 이름: AxiSlicer.Document

AxiSlicerView.h, AxiSlicerView.cpp - 문서 뷰
    이 파일에는 CAxiSlicerView 클래스가 들어 있습니다.
    CAxiSlicerView 개체는 CAxiSlicerDoc 개체를 보는 데 사용됩니다.

res\AxiSlicerDoc.ico
    CAxiSlicerDoc 클래스의 MDI 자식 창에 대한 아이콘으로 사용되는
    아이콘 파일입니다. 이 아이콘은 주 리소스 파일인 AxiSlicer.rc에
    의해 포함됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더 파일(PCH)인 AxiSlicer.pch와
